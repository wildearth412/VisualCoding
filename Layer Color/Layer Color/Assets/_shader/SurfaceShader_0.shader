Shader "Custom/SurfaceShader_0" {
	Properties {
		...
		_BlendMode("Blend Mode 0-24", Int) = 19
	...
	SubShader {
		...
		/* (_BlendMode == 0) { c = Opacity(); }
		 (_BlendMode == 1) { c = Dissolve(c2, c1, wos); }
		 (_BlendMode == 2) { c = Darken(c2, c1); }
		 (_BlendMode == 3) { c = Lighten(c2, c1); }
		 (_BlendMode == 4) { c = Multiply(c2, c1); }
		 (_BlendMode == 5) { c = Screen(c2, c1); }
		 (_BlendMode == 6) { c = ColorBurn(c2, c1); }
		 (_BlendMode == 7) { c = ColorDodge(c2, c1); }
		 (_BlendMode == 8) { c = LinearBurn(c2, c1); }
		 (_BlendMode == 9) { c = LinearDodge(c2, c1); }
		 (_BlendMode == 10) { c = Add(c2, c1); }
		 (_BlendMode == 11) { c = Remove(c2, c1); }
		 (_BlendMode == 12) { c = Overlay(c2, c1); }
		 (_BlendMode == 13) { c = HardLight(c2, c1); }
		 (_BlendMode == 14) { c = SoftLight(c2, c1); }
		 (_BlendMode == 15) { c = VividLight(c2, c1); }
		 (_BlendMode == 16) { c = LinearLight(c2, c1); }
		 (_BlendMode == 17) { c = PinLight(c2, c1); }
		 (_BlendMode == 18) { c = HardMix(c2, c1); }
		 (_BlendMode == 19) { c = Difference(c2, c1); }
		 (_BlendMode == 20) { c = Exclusion(c2, c1); }
		 (_BlendMode == 21) { c = Hue(c2, c1); }
		 (_BlendMode == 22) { c = Saturation(c2, c1); }
		 (_BlendMode == 23) { c = Color(c2, c1); }
		 (_BlendMode == 24) { c = Luminosity(c2, c1); }*/
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			if (_BlendMode == 0) { c = Opacity(cc1.rgb, c2, _Transparency1); }
						...
						Darken(c2, c1) * (_BlendMode == 2 ? 1.0f : 0) +
					    Lighten(c2, c1) * (_BlendMode == 3 ? 1.0f : 0) +
						Multiply(c2, c1) * (_BlendMode == 4 ? 1.0f : 0) +
						Screen(c2, c1) * (_BlendMode == 5 ? 1.0f : 0) +
						ColorBurn(c2, c1) * (_BlendMode == 6 ? 1.0f : 0) +
						ColorDodge(c2, c1) * (_BlendMode == 7 ? 1.0f : 0) +
						LinearBurn(c2, c1) * (_BlendMode == 8 ? 1.0f : 0) +
						LinearDodge(c2, c1) * (_BlendMode == 9 ? 1.0f : 0) +
						Add(c2, c1) * (_BlendMode == 10 ? 1.0f : 0) +
						Remove(c2, c1) * (_BlendMode == 11 ? 1.0f : 0) +
						Overlay(c2, c1) * (_BlendMode == 12 ? 1.0f : 0) +
						HardLight(c2, c1) * (_BlendMode == 13 ? 1.0f : 0) +
						SoftLight(c2, c1) * (_BlendMode == 14 ? 1.0f : 0) +
						VividLight(c2, c1) * (_BlendMode == 15 ? 1.0f : 0) +
						LinearLight(c2, c1) * (_BlendMode == 16 ? 1.0f : 0) +
						PinLight(c2, c1) * (_BlendMode == 17 ? 1.0f : 0) +
						HardMix(c2, c1) * (_BlendMode == 18 ? 1.0f : 0) +
						Difference(c2, c1) * (_BlendMode == 19 ? 1.0f : 0) +
						Exclusion(c2, c1) * (_BlendMode == 20 ? 1.0f : 0) +
						Hue(c2, c1) * (_BlendMode == 21 ? 1.0f : 0) +
						Saturation(c2, c1) * (_BlendMode == 22 ? 1.0f : 0) +
						Color(c2, c1) * (_BlendMode == 23 ? 1.0f : 0) +
						Luminosity(c2, c1) * (_BlendMode == 24 ? 1.0f : 0) ;  }

			c = ResultCheck(c);
			c = Normalize(c);

			/*c = RGB2HSV(c);
            c.x = (float)c.x + 0.5f * _Hfix - 1.0f * (float)floor(((float)c.x + 0.5f * _Hfix) / 1.0f) + 1.0f * (float)floor((-((float)c.x + 0.5f * _Hfix) + 1.0f) / 1.0f);
            c.y = (float)c.y + 0.5f * _Sfix - 1.0f * (float)floor(((float)c.y + 0.5f * _Sfix) / 1.0f) + 1.0f * (float)floor((-((float)c.y + 0.5f * _Sfix) + 1.0f) / 1.0f);
            c.z = (float)c.z + 0.5f * _Vfix - 1.0f * (float)floor(((float)c.z + 0.5f * _Vfix) / 1.0f) + 1.0f * (float)floor((-((float)c.z + 0.5f * _Vfix) + 1.0f) / 1.0f);
			c = HSV2RGB(c);*/

			c = RGB2HSV(c);
			c.x = c.x + (fixed)(0.5f * _Hfix);
			//if (c.x > 1.0f) c.x -= (fixed)1.0f;
			//if (c.x < 0) c.x += (fixed)1.0f;
			c.y = c.y + (fixed)(0.5f * _Sfix);
			//if (c.y > 1.0f) c.y -= (fixed)1.0f;
			//if (c.y < 0) c.y += (fixed)1.0f;
			c.z = c.z + (fixed)(0.5f * _Vfix);
			//if (c.z > 1.0f) c.z -= (fixed)1.0f;
			//if (c.z < 0) c.z += (fixed)1.0f;
			c = HSV2RGB(c);
