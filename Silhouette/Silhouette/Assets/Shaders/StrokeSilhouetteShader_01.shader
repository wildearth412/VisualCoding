...
Shader "Custom/StrokeSilhouette_01" 
{
	Properties
	{
		_MainTex("Texture", 2D) = "white" {}
		_Weight("Outline Weight", Range(1,4)) = 1
		_Threshold("Threshold", Range(0,0.1)) = 0.05    // i ki chi
			...
			fixed3 StrokeSilhouette(float2 uv)
			{
				...
				for (int i = -wt; i <= wt; i++)
				{
					for (int j = -wt; j <= wt; j++)
					{
						...
						{
							dif = true;
							float ds = length(float3((float)i, (float)j, 0));  // least UV offset distance to the different color point 
							mind = min(mind, ds);
						}
					}
				}
