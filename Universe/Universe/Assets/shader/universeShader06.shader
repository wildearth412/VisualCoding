Shader "universe/universeShader06"
{
	Properties
	{
		_MainTex ("Texture", 2D) = "white" {}
		zoom("Zoom Img",Range(0.001,10.)) = .003
	}
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
			    ...
			    // rending
			    float s = .25 * abs(sin(time * 2.025)), fade = 0.3;
			    ...
			    for(int r = 0; r < volsteps; r++){
				    ...
				    for (int i = 0; i < iterations; i++){
					    ...
					    //p = abs(p)/ pow(length(p),3.) - formuparam; 
					    p = abs(p.zxy)*abs(p.yzx)/ ( length(p) * length(p) * length(p) ) - formuparam;         //   ****************************** Feature *****************************
